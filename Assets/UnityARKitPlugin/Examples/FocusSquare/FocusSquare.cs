...
public class FocusSquare : MonoBehaviour {
	...
	public Material perfectMAT, partialMAT;
	...
	void Update () {
		...
		if (Physics.Raycast (ray, out hit, maxRayDistance, collisionLayerMask)) {
			...
			Debug.Log (string.Format ("x:{0:0.######} y:{1:0.######} z:{2:0.######}", foundSquare.transform.position.x, foundSquare.transform.position.y, foundSquare.transform.position.z));
			GetComponentInChildren<MeshRenderer>().material = partialMAT;
			GetComponentInChildren<MeshRenderer>().material = perfectMAT;
		...
		foreach (ARHitTestResultType resultType in resultTypes)
		{
			...
			{
			if(resultType == ARHitTestResultType.ARHitTestResultTypeHorizontalPlane){
				foundSquare.GetComponentInChildren<MeshRenderer>().material = perfectMAT;
			}
			else{
				foundSquare.GetComponentInChildren<MeshRenderer>().material = partialMAT;
			}
				SquareState = FocusState.Found;
				return;
			}
